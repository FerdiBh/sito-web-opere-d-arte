...
export default function Navbar({ cartCount, activeSection, setActiveSection, user }) {
  ...
  const links = [
    ...
    { key: 'login', label: user?.username || t('nav.login') },
  ]
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex items-center gap-8">
            {links.map((link) => (
              <button key={link.key} onClick={() => setActiveSection(link.key)} className={`text-sm font-medium transition-colors duration-200 ${activeSection === link.key ? 'text-gold' : 'text-gray-300 hover:text-gold'}`}>
                {link.label}
                {link.key === 'cart' && cartCount > 0 && <span className="ml-1 bg-gold text-darker text-xs rounded-full px-1.5 py-0.5">{cartCount}</span>}
              </button>
            ))}
            <LangBtn />
          </div>
        ...
        {menuOpen && (
          <div className="md:hidden pb-4 space-y-2">
            {links.map((link) => (
              <button key={link.key} onClick={() => { setActiveSection(link.key); setMenuOpen(false) }} className="block w-full text-left text-sm text-gray-300 hover:text-gold py-2">
                {link.label}
                {link.key === 'cart' && cartCount > 0 && <span className="ml-1 bg-gold text-darker text-xs rounded-full px-1.5 py-0.5">{cartCount}</span>}
              </button>
            ))}
            <div className="pt-2"><LangBtn /></div>
          </div>
        )}
      </div>
    </nav>
